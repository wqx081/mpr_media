...
sigset_t SetSignalMask(const sigset_t& new_sigmask) {
  ...
  CHECK(pthread_sigmask(SIG_SETMASK, &new_sigmask, &old_sigmask) == 0);
  return old_sigmask;
}
...
void ResetChildSignalHandlersToDefaults(void) {
  for (int signum = 1; ; ++signum) {
    ...
    if (sigaction_get_ret && errno == EINVAL) {
      ...
      CHECK(signum == kNumberOfSignals + 1);
      break;
    }
    // All other failures are fatal.
    if (sigaction_get_ret) {
      LOG(FATAL) << "sigaction (get) failed.";
    }
...
Process LaunchProcess(const std::vector<std::string>& argv,
  ...
  if (options.clone_flags) {
    ...
    // clone flags.
    CHECK((options.clone_flags & 0xff) == 0);
  ...
  if (pid < 0) {
    ...
    if (new_fd != STDIN_FILENO) {
      LOG(ERROR) << "Failed to dup /dev/null for stdin";
      _exit(127);
    }
...
#define PR_SET_NO_NEW_PRIVS 38
    ...
    if (current_directory != nullptr) {
      CHECK(chdir(current_directory) == 0);
    }
    ...
    LOG(ERROR) << "LaunchProcess: failed to execvp:";
    LOG(ERROR) << argv_cstr[0];
    _exit(127);
  } else {
    // Parent process
    if (options.wait) {
...
bool IsRunningOnValgrind() {
  return false; //RUNNING_ON_VALGRIND;
}
...
#endif
inline pid_t CloneAndLongjmpInChild(unsigned long flags,
                                      pid_t* ptid,
                                      pid_t* ctid,
                                      jmp_buf* env) {
...
pid_t ForkWithFlags(unsigned long flags, pid_t* ptid, pid_t* ctid) {
  ...
  if (clone_tls_used || invalid_ctid || invalid_ptid || clone_vm_used) {
    LOG(FATAL) << "Invalid usage of ForkWithFlags";
  }
