...
bool KillProcessGroup(ProcessHandle process_group_id) {
  ...
  if (!result)
    LOG(ERROR) << "Unable to terminate process group " << process_group_id;
  return result;
}
...
TerminationStatus GetKnownDeadTerminationStatus(ProcessHandle handle,
  ...
  if (!result)
    LOG(ERROR) << "Unable to terminate process " << handle;
