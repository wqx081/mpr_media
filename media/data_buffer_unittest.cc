...
TEST(DataBufferTest, Constructors) {
  ...
  scoped_ref_ptr<DataBuffer> buffer(new DataBuffer(0));
  EXPECT_FALSE(buffer->GetData());

  scoped_ref_ptr<DataBuffer> buffer2(new DataBuffer(kTestDataSize));
  EXPECT_EQ(0, buffer2->GetDataSize());
  EXPECT_EQ(kTestDataSize, buffer2->GetBufferSize());

  scoped_ref_ptr<DataBuffer> buffer3(new DataBuffer(kTestData, kTestDataSize));
...
TEST(DataBufferTest, ReadingWriting) {
  ...
  // Create a DataBuffer.
  scoped_ref_ptr<DataBuffer> buffer(new DataBuffer(kDataSize));
  ASSERT_TRUE(buffer);
  ...
  scoped_ref_ptr<DataBuffer> buffer2(new DataBuffer(kNewDataSize + 10));
