...
class AudioBusTest : public testing::Test {
 public:
  ...
  void VerifyBus(const AudioBus* result, const AudioBus* expected) {
    CHECK_EQ(expected->channels(), result->channels());
    CHECK_EQ(expected->frames(), result->frames());
  ...
  void VerifyChannelData(AudioBus* bus) {
    for (int i = 0; i < bus->channels(); ++i) {
      CHECK_EQ(0U, reinterpret_cast<uintptr_t>(
...
TEST_F(AudioBusTest, CreateUsingAudioParameters) {
  std::unique_ptr<AudioBus> bus = AudioBus::Create(AudioParameters(
...
TEST_F(AudioBusTest, WrapMemory) {
  ...
  std::unique_ptr<float, base::ScopedPtrAlignedFree> data(static_cast<float*>(
...
TEST_F(AudioBusTest, CopyTo) {
  ...
  }
  {
    ...
    std::unique_ptr<float, base::ScopedPtrAlignedFree> data(
...
TEST_F(AudioBusTest, FromInterleaved) {
  ...
  std::unique_ptr<AudioBus> expected = AudioBus::Create(
      kTestVectorChannels, kTestVectorFrames);
