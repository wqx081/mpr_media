...
//    GROW_ARRAY(fg->outputs, fg->nb_outputs);
    fg->outputs = (OutputFilter**) grow_array(fg->outputs,
                  sizeof(*fg->outputs),
                  &fg->nb_outputs,
                  fg->nb_outputs + 1);
    ...
    fg->inputs = (InputFilter**) grow_array(fg->inputs,
                                            sizeof(*fg->inputs),
                                            &fg->nb_inputs,
                                            fg->nb_inputs + 1);
...
//    GROW_ARRAY(ist->filters, ist->nb_filters);
    ist->filters = (InputFilter**) grow_array(ist->filters,
                   sizeof(*ist->filters),
                   &ist->nb_filters,
                   ist->nb_filters + 1);
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (in->name) {
        ...
        for (i = 0; (unsigned)i < s->nb_streams; i++) {
...
int init_complex_filtergraph(FilterGraph *fg)
{
    ...
    *      * and outputs we have, and is discarded on exit from this function */
    ...
    for (cur = outputs; cur;) {
        ...
        fg->outputs[fg->nb_outputs - 1]->type    = avfilter_pad_get_type(cur->filter_ctx->output_pads, cur->pad_idx);
        cur = cur->next;
...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg) do {                 \
    ...
    if (ost->apad && of->shortest) {
        ...
        if ((unsigned)i<of->ctx->nb_streams) {
...
static int sub2video_prepare(InputStream *ist)
{
    ...
    if (!(w && h)) {
        for (i = 0; (unsigned)i < avf->nb_streams; i++) {
