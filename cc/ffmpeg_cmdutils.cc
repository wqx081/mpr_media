...
int opt_default(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!(p = strchr(opt, ':')))
        p = opt + strlen(opt);
    av_strlcpy(opt_stripped, opt, FFMIN(sizeof(opt_stripped), (unsigned long)(p - opt + 1)));
...
int opt_loglevel(void *optctx, const char *opt, const char *arg)
{
  ...
  (void) opt;
    ...
    for (i = 0; (unsigned)i < FF_ARRAY_ELEMS(log_levels); i++) {
        ...
        if ((ret = av_opt_get_key_value(&env, "=", ":", 0, &key, &val)) < 0) {
            if (count)
                av_log(NULL, AV_LOG_ERROR,
                       ...
                       av_make_error_string(errbuf, 512, ret));
            break;
        }
        if (*env)
            env++;
