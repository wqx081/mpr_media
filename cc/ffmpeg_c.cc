...
// Helpers
static inline char* AvErr2Str(int errnum) {
  char errbuf[512] = {0};
  return av_make_error_string(errbuf, 512, errnum);
}

static inline char* AvTs2Str(int64_t ts) {
  char buf[512] = {0};
  return av_ts_make_string(buf, ts);
}

static inline char* AvTs2TimeStr(int64_t ts, AVRational* tb) {
  char buf[512] = {0};
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        for (i = 0; (unsigned)i<FF_ARRAY_ELEMS(ost->error); i++) {
            if (sd && i < sd[5])
    ...
    if (ret < 0) {
        ...
        close_all_output_streams(ost,
                                 (OSTFinished)((int)MUXER_FINISHED | (int)ENCODER_FINISHED),
                                 ENCODER_FINISHED);
    }
    av_packet_unref(pkt);
}
                         ...
                         AVFrame *frame)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "encoder <- type:audio "
               ...
               AvTs2Str(frame->pts), AvTs2TimeStr(frame->pts, &enc->time_base),
                         ...
                         double sync_ipts)
{
    ...
    if (!next_picture) {
        ...
        nb0_frames = 0; // tracks the number of times the PREVIOUS frame should be duplicated, mostly for variable fram erate (VFR)
        nb_frames = 1;
        ...
        if (format_video_sync == VSYNC_AUTO) {
            if(!strcmp(s->oformat->name, "avi")) {
                ...
                format_video_sync = (s->oformat->flags & AVFMT_VARIABLE_FPS) ? ((s->oformat->flags & AVFMT_NOTIMESTAMPS
...
#endif
        {
            ...
            if (debug_ts) {
                av_log(NULL, AV_LOG_INFO, "encoder <- type:video "
                       "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
                       AvTs2Str(in_picture->pts), AvTs2TimeStr(in_picture->pts, &enc->time_base),
                       enc->time_base.num, enc->time_base.den);
            }
            ...
            if (got_packet) {
                if (debug_ts) {
                    av_log(NULL, AV_LOG_INFO, "encoder -> type:video "
                           "pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s\n",
                           AvTs2Str(pkt.pts), AvTs2TimeStr(pkt.pts, &enc->time_base),
                           AvTs2Str(pkt.dts), AvTs2TimeStr(pkt.dts, &enc->time_base));
                }
...
inline OSTFinished FinishedOr(OSTFinished a, OSTFinished b) {
    int r = (int)a | (int) b;
    return (OSTFinished)(r);
}
...
static void finish_output_stream(OutputStream *ost)
{
    ...
    ost->finished = FinishedOr(ENCODER_FINISHED , MUXER_FINISHED);

    if (of->shortest) {
        for (i = 0; (unsigned)i < of->ctx->nb_streams; i++)
            output_streams[of->ost_index + i]->finished = FinishedOr(ENCODER_FINISHED , MUXER_FINISHED);
    }
}
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    av_log(NULL, AV_LOG_INFO, "filter -> pts:%s pts_time:%s exact:%f time_base:%d/%d\n",
                           AvTs2Str(filtered_frame->pts), AvTs2TimeStr(filtered_frame->pts, &enc->time_base),
                           float_pts,
                           enc->time_base.num, enc->time_base.den);
                }
...
static void print_final_stats(int64_t total_size)
{
    ...
    if (data_size && total_size>0 && (uint64_t)total_size >= data_size)
        percent = 100.0 * (total_size - data_size) / data_size;

    av_log(NULL, AV_LOG_INFO, "video:%1.0fkB audio:%1.0fkB subtitle:%1.0fkB other streams:%1.0fkB global headers:%1.0fk B muxing overhead: ",
           video_size / 1024.0,
           audio_size / 1024.0,
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        for (j = 0; (unsigned)j < of->ctx->nb_streams; j++) {
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (qp_hist) {
                int j;
                int qp = lrintf(q);
                if (qp >= 0 && (unsigned)qp < FF_ARRAY_ELEMS(qp_histogram))
                    qp_histogram[qp]++;
                for (j = 0; j < 32; j++)
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (ist->fix_sub_duration) {
        ...
        if (ist->prev_sub.got_output) {
                             ...
                             1000, AV_TIME_BASE);
            if ((uint32_t)end < ist->prev_sub.subtitle.end_display_time) {
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if (ost->enc_ctx->nb_coded_side_data) {
            ...
            ost->st->side_data = (AVPacketSideData *)av_realloc_array(NULL, ost->enc_ctx->nb_coded_side_data, sizeof(*ost->st->side_data));
            if (!ost->st->side_data)
                return AVERROR(ENOMEM);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!memcmp(p, "chapters", 8)) {
            ...
            if (avf->nb_chapters > INT_MAX - size ||
                    !(pts = (int64_t *)av_realloc_f(pts, size += avf->nb_chapters - 1, sizeof(*pts)))) {
                av_log(NULL, AV_LOG_FATAL,
            ...
            for (j = 0; (unsigned)j < avf->nb_chapters; j++) {
...
static void report_new_stream(int input_index, AVPacket *pkt)
{
    ...
    av_log(file->ctx, AV_LOG_WARNING,
           ...
           pkt->pos, AvTs2TimeStr(pkt->dts, &st->time_base));
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ist) {
            ...
            for (j=0; (unsigned)j<oc->nb_streams; j++) {
                AVStream *st = oc->streams[j];
                if (st != ost->st && st->codec->codec_type == enc_ctx->codec_type)
                    break;
            }
            if ((unsigned)j == oc->nb_streams)
        ...
        if (ost->stream_copy) {
            ...
            if (ist->st->nb_side_data) {
                ost->st->side_data = (AVPacketSideData *)av_realloc_array(NULL, ist->st->nb_side_data, sizeof(*ist->st->side_data));
                if (!ost->st->side_data)
                    return AVERROR(ENOMEM);
...
#define INIT_OPTS(index, n, t, i, min_v, max_v) \
    opts1[index].name = n; opts1[index].help = NULL; opts1[index].offset = 0; opts1[index].type = t; opts1[index].default_val.i64 = i; opts1[index].min=min_v; opts1[index].max=max_v; opts1[index].unit = "flags"

        INIT_OPTS(0, "disposition",     AV_OPT_TYPE_FLAGS, 0, INT64_MIN, INT64_MAX);
        INIT_OPTS(1, "default",         AV_OPT_TYPE_CONST, AV_DISPOSITION_DEFAULT, 0, 0);
        INIT_OPTS(2, "dub",             AV_OPT_TYPE_CONST, AV_DISPOSITION_DUB, 0, 0);
        INIT_OPTS(3, "original",        AV_OPT_TYPE_CONST, AV_DISPOSITION_ORIGINAL, 0, 0);
        INIT_OPTS(4, "comment",         AV_OPT_TYPE_CONST, AV_DISPOSITION_COMMENT, 0, 0);
        INIT_OPTS(5, "lyrics",          AV_OPT_TYPE_CONST, AV_DISPOSITION_LYRICS, 0, 0);
        INIT_OPTS(6, "karaoke",         AV_OPT_TYPE_CONST, AV_DISPOSITION_KARAOKE, 0, 0);
        INIT_OPTS(7, "forced",          AV_OPT_TYPE_CONST, AV_DISPOSITION_FORCED, 0, 0);
        INIT_OPTS(8, "hearing_impaired",AV_OPT_TYPE_CONST, AV_DISPOSITION_HEARING_IMPAIRED, 0, 0);
        INIT_OPTS(9, "visual_impaired", AV_OPT_TYPE_CONST, AV_DISPOSITION_VISUAL_IMPAIRED, 0, 0);
        INIT_OPTS(10, "clean_effects",  AV_OPT_TYPE_CONST, AV_DISPOSITION_CLEAN_EFFECTS, 0, 0);
        INIT_OPTS(11, "captions",       AV_OPT_TYPE_CONST, AV_DISPOSITION_CAPTIONS, 0, 0);
        INIT_OPTS(12, "descriptions",   AV_OPT_TYPE_CONST, AV_DISPOSITION_DESCRIPTIONS, 0, 0);
        INIT_OPTS(13, "metadata",       AV_OPT_TYPE_CONST, AV_DISPOSITION_METADATA, 0, 0);
        INIT_OPTS(14, NULL,             AV_OPT_TYPE_CONST, 0, 0, 0); // the last one 
...
#undef INIT_OPTS
            ...
            const AVClass *pclass = &cls;

            ret = av_opt_eval_flags(&pclass, &opts1[0], ost->disposition, &ost->st->disposition);
            if (ret < 0)
                goto dump_format;
        }
    }
    ...
    for (i = 0; i < nb_input_files; i++) {
        InputFile *ifile = input_files[i];
        for (j = 0; (unsigned)j < ifile->ctx->nb_programs; j++) {
            AVProgram *p = ifile->ctx->programs[j];
            int discard  = AVDISCARD_ALL;

            for (k = 0; (unsigned)k < p->nb_stream_indexes; k++)
...
static int need_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->finished ||
                (os->pb && (uint64_t)avio_tell(os->pb) >= of->limit_filesize))
            continue;
        if (ost->frame_number >= ost->max_frames) {
            int j;
            for (j = 0; (unsigned)j < of->ctx->nb_streams; j++)
...
static int process_input(int file_index)
{
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64) {
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            for (i=0; (unsigned)i<is->nb_streams; i++) {
...
static int transcode(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error writing trailer of %s: %s", os->filename, AvErr2Str(ret));
            if (exit_on_error)
                exit_program(1);
        }
    }
